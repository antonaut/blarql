	...
	for(var i=0; i<bindings.length;i++){
		...
		if(!oc){
			...
			if(!o['xml:lang'] || o['xml:lang'] && (o['xml:lang'] === 'en' || o['xml:lang'] === 'fr')){
				...
				o.name = p.value.replace(/http:\/\/.+\//gi,'') + " - " + o.name;
				o.size = 10;
			}
		}
...
function askForQueryResult(hash){
	...
	if(jsonTriples.errId){
		...
		console.log(JSON.stringify(jGraph));
		$.ajax({
		  type: 'POST',
		  url: 'http://www.thoretton.com:80/H/websem/php/plopJSON.php',//url of receiver file on server
		  data: {
			data: 	JSON.stringify(jGraph), //your data
		  },
		  success: function() {
			loadGraph();
		  },
...
var mainMethod = function(){
	...
	var search = decodeURIComponent(QueryString.search);
	console.log('search '+search);

	if(keywords != "undefined" && search != "undefined"){
		websem(keywords);
	}
}();
